  ...
  for (const auto& fused_node_and_graph : fused_nodes_and_graphs) {
    ...
    compute_info.compute_func = [](FunctionState state, const OrtCustomOpApi* api, OrtKernelContext* context) {
      ...
      // TODO, investigate concurrent runs for different executions from the same model
      {
        ...
        std::cout<<"nnapi set input and output time: ";
        ...
        std::cout << "nnapi compute time: ";
        auto end1 = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double,std::ratio<1,1>> ds1 = end1 - end;
        std::chrono::milliseconds d1 = std::chrono::duration_cast< std::chrono::milliseconds >( ds1 );
        std::cout<<d1.count()<<"ms   ";
        std::chrono::duration<double,std::ratio<1,1000000>> duration_mcs1=std::chrono::duration_cast<std::chrono::duration<double,std::ratio<1,1000000>>> (end1 - end);  
        std::cout<<duration_mcs1.count()<<"microseconds"<<std::endl; 
